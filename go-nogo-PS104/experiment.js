...
//Changed by Tom to get aggregate RT and acc data when in debrief mode
...
// Tom changed to reflect shorter experiment
...
var test_trials = jsPsych.randomization.repeat(test_stimuli_block, 4);   
...
//New feedback block from Tom/jpsych tut using stuff from assessPerformance function
var debrief_block = {
  type: 'poldrack-text',
  text: function() {

	var experiment_data = jsPsych.data.getTrialsOfType('poldrack-single-stim')
	var wrong_count = 0 //count trials where we incorrectly go or no go
	var trial_count = 0
	var rt_array = []
	var rt = 0
	for (var i = 0; i < experiment_data.length; i++) {
		if (experiment_data[i].possible_responses != 'none') {
			trial_count += 1
			if (experiment_data[i].correct == false) {
				wrong_count +=1
			}
			rt = experiment_data[i].rt
			if (rt != -1) {
	...
	var accuracy = Math.round( (trial_count - wrong_count)/trial_count *100);
    return `<div class = centerbox><p class = center-block-text>You responded correctly on ${accuracy}% of the trials.</p>
      <p class = center-block-text>Your average response time was ${avg_rt} milliseconds.</p>
      <p class = center-block-text><strong>Make a note of these numbers now!</strong></p>
...
go_nogo_experiment.push(attention_node)
go_nogo_experiment.push(debrief_block)
go_nogo_experiment.push(debrief_conf)
